...
const MealItem = ({ meal, remove }) => {
  ...
  return (
    <>
      <div
        className={`p-4 bg-yellow-100 mx-4 my-4 relative  max-w-md  text-center  shadow-xl transition-all rounded-lg box ${
          boxTop < trigger ? 'show' : ''
        }`}
        ref={box}
      >
        ...
        {meal.kategorija && (
          <p className='text-gray-50 bg-green-500 text-center inline-block my-2 text-base py-2 px-4 '>
            {meal.kategorija || ''}
          </p>
        )}
        <div className='w-full p-4 flex align-center justify-center flex-col mx-auto my-auto bg-yellow-50 shadow-lg rounded-lg'>
          ...
          <div className='text-center py-2'>
            <Link to={`/${meal.id}`}>
              <button className='bg-yellow-500 px-4 py-2 rounded-lg text-gray-50 w-4/5 font-medium mt-2 hover:bg-red-500 transition-all'>
                Detalji
              </button>
            </Link>
          </div>
